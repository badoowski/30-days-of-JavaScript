// Exercise Level 1

/* 
 Question 1: Get user input using prompt(“Enter your age:”). If user is 18 or older , give feedback:'You are old
  enough to drive' but if not 18 give another feedback stating to  wait for the number of years he needs to
   turn 18.

   ```sh
   Enter your age: 30
   You are old enough to drive.

   Enter your age:15
   You are left with 3 years to drive.
   ```

*/

// Answer
let userAge = parseInt(prompt('Enter your age'));
let lessAge = 18 - userAge;
if(userAge >= 18){
   console.log('You are old enough to drive');
}
else{
   console.log('You are left with ' + lessAge + ' years to drive');
}

/* 
 Question 2: Compare the values of myAge and yourAge using if … else. Based on the comparison and log the result to 
 console stating who is older (me or you). Use prompt(“Enter your age:”) to get the age as input.

   ```sh
   Enter your age: 30
   You are 5 years older than me.
   ```

*/

// Answer
let myAge = parseInt(prompt('Enter my age'));
let yourAge = parseInt(prompt('Enter your age'));
const ageDiff = myAge - yourAge;
const ageDiff2 = yourAge - myAge;
if (myAge===yourAge){
    console.log('We are both ' + myAge + ' years old');
}
else if (myAge<yourAge){
    console.log('You are ' + ageDiff2 + ' years older than me');
}
else{
console.log('I am ' + ageDiff + ' years older than you');
}

/* 
 Question 3: If a is greater than b return 'a is greater than b' else 'a is less than b'. Try to implement it in
  to ways

    - using if else
    - ternary operator.

    ```js
      let a = 4
      let b = 3
    ```

    ```sh
      4 is greater than 3
    ```
*/

// Answer
let a = 4;
let b = 3;
if (a > b){
    console.log('a is greater than b');
}else{
    console.log('a is less than b');
}

/* 
 Question 4: Even numbers are divisible by 2 and the remainder is zero. How do you check, if a number is even or not using JavaScript?

    ```sh
    Enter a number: 2
    2 is an even number

    Enter a number: 9
    9 is is an odd number.
    ```
*/

// Answer
let num = parseInt(prompt('Enter a number'));
if (num % 2 === 0){
    console.log(num + ' is even');
}else{
    console.log(num + ' is odd');

}